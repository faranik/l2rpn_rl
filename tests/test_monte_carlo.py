...
class TestMonteCarlo(unittest.TestCase):
    ...
    def test_init_state_space_size_initialization(self):
        mdp = model.MonteCarlo(10, 0.0, 1, 0.5)
        self.assertEqual(mdp.action_value_fn.size, 10)

    def test_init_value_fn_initialization(self):
        mdp = model.MonteCarlo(10, 0.0, 0, 0.5)
        comparing_array = np.zeros(10, float)
        self.assertTrue(np.array_equal(comparing_array, mdp.action_value_fn))

    def test_learn(self):
        mdp = model.MonteCarlo(5, 1, 5, 0.5)
        history = ((1, 1.0), (4, 1.5), (0, 0.9))

        mdp.learn(history)

        self.assertEqual(mdp.action_value_fn[0], 5.4)
        self.assertEqual(mdp.action_value_fn[1], 1.0)
        self.assertEqual(mdp.action_value_fn[2], 0.0)
        self.assertEqual(mdp.action_value_fn[3], 0.0)
        self.assertEqual(mdp.action_value_fn[4], 3.0)

    def test_is_mature_returns_false_if_untrained(self):
        mdp = model.MonteCarlo(2, 0.1, 1, 0.5)
        self.assertFalse(mdp.is_mature())

    def test_is_mature_returns_true_if_trained(self):
        mdp = model.MonteCarlo(5, 0.1, 1, 0.5)
        history = ((1, 1.0), (4, 1.5), (0, 0.9))
